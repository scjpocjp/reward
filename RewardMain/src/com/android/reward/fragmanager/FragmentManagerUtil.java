package com.android.reward.fragmanager;
...
import com.android.reward.fragment.MainFragment;
...
/*		if ( fragmentMap.containsKey( fragmentName ) ) {
		...
		*/
			...
			//	File root = new File("com.android.reward.fragment");
	...
	private void startFragment ( final FragmentHolder fragmentHolder, final Bundle args, int resourceId, boolean animate ) {
		...
		InputMethodManager inputManager = (InputMethodManager) Constants.appInstance .getSystemService(Context.INPUT_METHOD_SERVICE);
		
		inputManager.hideSoftInputFromWindow(  Constants.mBinder, InputMethodManager.HIDE_NOT_ALWAYS );
		
		Constants.isCurrent = true;
		if ( Constants.isCurrent ) {
			if ( ft == null ) {
				ft  = RewardApplication.getFragmentManager().beginTransaction();
			}
			if ( resourceId == -1 )
			{
			...
			if ( Constants.context != null && resourceId != -1 ) {
				LinearLayout li = (LinearLayout) MainActivity.context.findViewById( resourceId );
				if ( li != null ) {
					li.removeAllViews();
				}
			}
	...
	public boolean removeFragment ( String fragmentName ) {
		...
		FragmentTransaction ft  = RewardApplication.getFragmentManager().beginTransaction();
	...
	public void popBackStack () {
		if ( Constants.isCurrent ) {
			FragmentManager fragmentManager = RewardApplication.getFragmentManager();
